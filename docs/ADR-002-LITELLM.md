...
LiteLLM source is a pinned Git submodule of `DivyanKavdia/litellm`, so upgrades and fork changes remain reviewable. The initial upstream image is separately pinned by digest. A fork build must use the reviewed source revision and publish a new digest; changing the submodule alone does not alter an already built image. Keep upstream changes small and run the complete integration suite for every upgrade.

The user created `DivyanKavdia/litellm`. On 13 September 2026, its `BerriAI/litellm` parent and the availability of the tested source commit were verified. `npm run litellm:source -- --use DivyanKavdia/litellm` fetched the pinned commit, linked the fork and marked the source lock as created. The script also supports verifying another fork or creating one with a locally supplied GitHub token.
